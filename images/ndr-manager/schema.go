...
var fieldDefs = []FieldSpec{
	...
	{Key: "xdr.task_token", Label: "结构化检索任务令牌", Type: "secret", Group: "xdr", Order: 44, Section: cfgXdr,
		Help: "XDR 平台调用 POST /api/xdr/task 时的 Bearer 令牌；NDR 在本地元数据上完成检索/关联分析"},
	{Key: "xdr.agent_task_token", Label: "Agent 研判任务令牌", Type: "secret", Group: "xdr", Order: 44, Section: cfgXdr,
		Help: "XDR 平台调用 POST /api/xdr/agent/task 时的 Bearer 令牌；与 task_token 分离，便于权限控制。Agent 研判任务会消耗本地 LLM 算力，建议独立管理"},
	{Key: "xdr.agent_token", Label: "Agent 服务鉴权令牌", Type: "secret", Group: "xdr", Order: 45, Section: cfgXdr,
		Help: "ndr-manager 转发请求到 Agent /analyze 时使用的 Bearer 令牌；Agent 端用 AGENT_TOKEN 环境变量配置相同值"},
	{Key: "xdr.agent_enabled", Label: "启用本地分析 Agent", Type: "bool", Group: "xdr", Order: 45, Section: cfgXdr,
		Help: "开启后 /api/xdr/agent/task 由本地 Agent（小模型 + MCP 工具）自主分析；关闭则仅结构化任务接口可用"},
	{Key: "xdr.agent_url", Label: "Agent 服务地址", Type: "string", Group: "xdr", Order: 46, Section: cfgXdr,
		Help: "本地分析 Agent 的 HTTP 服务地址（默认 http://nss-ndr-agent:8081/analyze）"},
