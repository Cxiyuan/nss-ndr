...
var cleanerCfg cleanerConfig

func loadCleanerConfig() error {
	data, err := os.ReadFile(filepath.Join(confDir, "probe.yaml"))
	if err != nil {
		return err
	}
	if err := yaml.Unmarshal(data, &cleanerCfg); err != nil {
		return err
	}
	if cleanerCfg.Probe.MinFreeGB == 0 {
		cleanerCfg.Probe.MinFreeGB = 20
	}
	if cleanerCfg.Probe.DiskPressureThreshold == 0 {
		cleanerCfg.Probe.DiskPressureThreshold = 90
	}
	return nil
}
...
func runCleanerOnce() {
	...
	st.FSUsagePct = usage
	if usage > cleanerCfg.Probe.DiskPressureThreshold {
		st.PressureTriggered = true
		log.Printf("磁盘用量 %d%% 超过阈值 %d%%，触发压力清理", usage, cleanerCfg.Probe.DiskPressureThreshold)
		cleanUnderPressure()
		usage = fsUsagePct(nsmDir)
		st.FSUsagePct = usage
	}
	...
	if int(freeGB) < cleanerCfg.Probe.MinFreeGB {
		log.Printf("WARN: /nsm 剩余 %.1fGB 低于 min_free_gb=%d", freeGB, cleanerCfg.Probe.MinFreeGB)
	}
...
func cleanupRoutine() (int, int64, []string) {
	...
	delEve, delBytes := removeOldFiles(suricataEv, "eve-*.json", cleanerCfg.Suricata.Eve.RetentionDays)
	totalFiles += delEve
	totalBytes += delBytes

	delZeek, zbytes := removeOldDirs(zeekLogs, cleanerCfg.Zeek.HistoryRetentionDays)
	totalFiles += delZeek
	totalBytes += zbytes

	delExt, ebytes := removeOldFiles(zeekExt, "*", cleanerCfg.Zeek.Extraction.MaxDays)
	totalFiles += delExt
	totalBytes += ebytes

	// Strelka 已扫描文件与结果日志（history 由 filecheck 内部按天清理，参照 SO cron）
	if cleanerCfg.Strelka.Enabled {
		delProc, pbytes := removeOldFiles(strelkaProc, "*", cleanerCfg.Strelka.Retention.ProcessedDays)
		totalFiles += delProc
		totalBytes += pbytes
		delLog, lbytes := removeOldFiles(strelkaLog, "strelka*.log*", cleanerCfg.Strelka.Retention.LogDays)
		totalFiles += delLog
		totalBytes += lbytes
	}

	// 全包：先按天数，再按总量上限
	pcapDays := cleanerCfg.Suricata.Pcap.RetentionDays
	if pcapDays > 0 {
		delP, pbytes := removeOldFiles(suripcap, "so-pcap.*", pcapDays)
		totalFiles += delP
		totalBytes += pbytes
	}
	limit := int64(cleanerCfg.Suricata.Pcap.StorageLimitGB) * 1e9
	if limit > 0 {
...
func cleanUnderPressure() {
	for _, target := range []struct {
		...
		for fsUsagePct(nsmDir) > cleanerCfg.Probe.DiskPressureThreshold {
			var removed int64
...
// startCleaner 数据清理调度（后台 goroutine）：按 cleanup_interval 周期执行常规清理 + 磁盘压力兜底
func startCleaner() {
	go func() {
		time.Sleep(10 * time.Second)
		interval := time.Hour
		_ = loadCleanerConfig()
		if d, err := time.ParseDuration(cleanerCfg.Probe.CleanupInterval); err == nil && d > 0 {
			interval = d
		}
		for {
			runCleanerOnce()
